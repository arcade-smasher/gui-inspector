	...
	public static String getMappedName(Class<?> clazz, String methodName) {
		...
		if (methodName.startsWith("method_")) {
			...
			if (superclass != null && superclass != Object.class) {
				String superMapped = getMappedName(superclass, methodName);
				if (!superMapped.equals(methodName)) return superMapped;
			}
			for (Class<?> iface : clazz.getInterfaces()) {
				String ifaceMapped = getMappedName(iface, methodName);
				if (!ifaceMapped.equals(methodName)) return ifaceMapped;
			}
		}
